...
选定一条线段作为长度单位, 我们可以量得一条线段的长度,线段 \({AB}\) 的长度,就是有向线段 \(\overline{AB}\) 的长度,记作 \(\left| {AB}\right|\) . 如图 1-3,设线段 \(e\) 是长度单位,那么 \(\left| {AB}\right| = 3\) . 因为有向线段的长度与它的方向无关,所以 \(\left| {AB}\right| = \left| {BA}\right|\) .
...
1. 数轴上点 \(A\) 的坐标为 2,点 \(B\) 的坐标为 -3 . 验证公式 \({AB} = {x}_{2} - {x}_{1}.\)

2. 已知数轴 \(x\) 上的点 \(A\text{、}B\text{、}C\) 的坐标分别为 \(1\text{、}2\text{、}3\) . (1) 求 \(\overline{AB}\text{、}\overline{CB}\) 的数量; (2) 如果在 \(x\) 轴上还有两个点 \(D\text{、}E\) ,且 \({AD} = {2.5},{CE} = - 3\) . 求点 \(D\text{、}E\) 的坐标.
...
4. 已知点 \(A\left( {a, - 5}\right)\) 和 \(B\left( {0,{10}}\right)\) 的距离是 17,求 \(a\) 的值.
...
1. 已知两点 \({P}_{1}\left( {3, - 2}\right) \text{、}{P}_{2}\left( {-9,4}\right)\) . 求点 \(P\left( {x,0}\right)\) 分 \(\overline{{P}_{1}{P}_{2}}\) 所成的比 \(\lambda\) 及 \(x\) 的值.

2. 点 \(M\) 分有向线段 \(\overline{{M}_{1}{M}_{2}}\) 的比为 \(\lambda\) ,求点 \(M\) 的坐标 \(\left( {x,y}\right)\) :
...
3. 已知 \(\bigtriangleup {ABC}\) 的顶点 \(A\left( {2,3}\right) \text{、}B\left( {8, - 4}\right)\) 和重心 \(G\left( {2, - 1}\right)\) . 求点 \(C\) 的坐标 \(\left( {x,y}\right)\) .

\section*{习 题 一}

1. 如图, 数轴上每一格等于一个长度单位,说出有向线段 \(\overline{AB}\) 、
...
2. 已知数轴上 \(A\text{、}B\) 两点的坐标 \({x}_{1}\text{、}{x}_{2}\) 分别是:
...
3. \(A\text{、}B\) 是数轴上两点,点 \(B\) 的坐标是 \({x}_{2}\) . 根据下列条件, 求点 \(A\) 的坐标 \({x}_{1}\) :

(1) \({x}_{2} = 3,{AB} = 5\) ; (2) \({x}_{2} = - 5,{BA} = - 3\) ;

(3) \({x}_{2} = 0,\left| {AB}\right| = 2\) ; (4) \({x}_{2} = - 5,\left| {AB}\right| = 2\) .

4. 已知某零件一个面上有 3 个孔, 孔中心的坐标分别为: \(A\left( {-{10},{30}}\right) \text{、}B\left( {-2,3}\right) \text{、}C\left( {0, - 1}\right)\) . 求每两孔中心的距离.

5. 已知点 \(P\left( {x,2}\right) \text{、}Q\left( {-2, - 3}\right) \text{、}M\left( {1,1}\right)\) ,且 \(\left| {PQ}\right| = \left| {PM}\right|\) . 求 \(x\) .

6. (1) 求在 \(x\) 轴上与点 \(A\left( {5,{12}}\right)\) 的距离为 13 的点的坐标;

（2）已知点 \(P\) 的横坐标是 7,点 \(P\) 到点 \(N\left( {-1,5}\right)\) 的距离等于 10,求点 \(P\) 的纵坐标.

7. 设线段 \({P}_{1}{P}_{2}\) 长 \(5\mathrm{\;{cm}}\) ,写出点 \(P\) 分 \(\overline{{P}_{1}{P}_{2}}\) 所成的比 \(\lambda\) :
...
9. 一条线段的两个端点 \({P}_{1}\text{、}{P}_{2}\) 的坐标及点 \(P\) 分 \(\overline{{P}_{1}{P}_{2}}\) 所成的比如下,求分点 \(P\) 的坐标:
...
10. (1) 一条线段的两个端点坐标如下, 求这条线段的两个三等分点的坐标: (i) \(\left( {-1,2}\right) \text{、}\left( {-{10}, - 1}\right)\) ; (ii) \(\left( {7,8}\right) \text{、}\left( {1, - 6}\right)\) .

（2）已知点 \(A\left( {1, - 1}\right) \text{、}B\left( {-4,5}\right)\) . 将线段 \({AB}\) 延长至 \(C\) , 使 \(\left| {AC}\right| = 3\left| {AB}\right|\) . 求点 \(C\) 的坐标.

11. 三角形的三个顶点是 \(A\left( {2,1}\right) \text{、}B\left( {-2,3}\right) \text{、}C\left( {0, - 1}\right)\) . 求三条中线的长度.

12. 已知点 \({P}_{1}\) 和 \({P}_{2}\) 的坐标分别是 \(\left( {4, - 3}\right)\) 和 \(\left( {-2,6}\right)\) ,求适合下列条件的点 \(P\) 的坐标:
...
13. (1) 已知三点 \(A\left( {x,5}\right) \text{、}B\left( {-2,y}\right) \text{、}C\left( {1,1}\right)\) ,且点 \(C\) 平分线段 \({AB}\) . 求 \(x\text{、}y\) .

(2) 已知两点 \(A\left( {3, - 1}\right) \text{、}B\left( {2,1}\right)\) ,求点 \(A\) 关于点 \(B\) 的对称点的坐标.

14. 已知三点 \(A\left( {1, - 1}\right) \text{、}B\left( {3,3}\right) ,C\left( {4,5}\right)\) . 求证: 三点在一条直线上.

15. (1) 证明: 直角三角形斜边的中点到三个顶点的距离相等;

(2) 证明: 三角形中位线等于底边的一半.

\section*{二 直线的方程}

\section*{1.3 一次函数的图象与直线的方程}
...
2. (1) 用量角器测量上题中各直线向上的方向与 \(x\) 轴的正方向所成的角的度数;

(2) 量出各直线与 \(x\) 轴、 \(y\) 轴交点的坐标,并代入方程, 看它们是不是方程的解.

\section*{1. 4 直线的倾斜角和斜率}
...
3. 已知: \(a\text{、}b\text{、}c\) 是两两不等的实数. 求经过下列每两个点的直线的倾斜角:
...
4. 证明: 已知三点 \(A\text{、}B\text{、}C\) . 如果直线 \({AB}\text{、}{AC}\) 的斜率相同. 那么这三点在同一条直线上.

\section*{1.5 直线方程的几种形式}
...
2. 已知下列直线的点斜式方程, 求各直线经过的已知点、直线的斜率和倾斜角:
...
1. 在什么情况下直线方程可以表示成下列形式:

(1) 点斜式; (2) 斜截式; (3) 两点式; (4) 截距式.

2. 求过下列两点的直线的两点式方程, 再化成斜截式方程:
...
2. 已知 \(y < {kx} + b\) 表示直线 \(y = {kx} + b\) 下方的半平面,说明当 \(B < 0\) 时,不等式 \({Ax} + {By} + C > 0\) 表示的区域是直 线 \({Ax} + {By} + C = 0\) 下方的半平面.

3. 已知每袋面粉 50 斤, 每袋大米 210 斤, 用栽重量是 530 斤的小推车运输, 找出车运效率最高的装车方案.
...
1. 丙烯腈是合成人造羊毛的原料. 在研究它的比重 \(D\) 和温度 \(T\left( {{}^{ \circ }\mathrm{C}}\right)\) 的关系时,通过实验得到的数据如下:
...
2. 根据下列实验数据,用平均值法求关于 \(m\text{、}p\) 的经验公式:
...
1. 四边形 \({ABCD}\) 的四个顶点是 \(A\left( {2,3}\right) \text{、}B\left( {1, - 1}\right) \text{、}C( - 1\) , \(- 2)\text{、}D\left( {-2,2}\right)\) . 求四边所在直线的倾斜角和斜率.
...
3. 已知直线的斜率 \(k = 2,{P}_{1}\left( {3,5}\right) \text{、}{P}_{2}\left( {{x}_{2},7}\right) \text{、}{P}_{3}\left( {-1,{y}_{3}}\right)\) 是这条直线上的三个点. 求 \({x}_{2}\) 和 \({y}_{8}\) .

4. 已知两点 \(M\left( {2,2}\right)\) 和 \(N\left( {5, - 2}\right)\) ,点 \(P\) 在 \(x\) 轴上,且 \(\angle {MPN} = {Rt}\angle\) . 求点 \(P\) 的坐标.

5. 设 \(A\text{、}B\) 两点的坐标分别是 \(\left( {{x}_{1},{y}_{1}}\right)\) 和 \(\left( {{x}_{2},{y}_{2}}\right)\) ,直线 \({AB}\) 的倾斜角是 \(\alpha\) . 求证:

\[
  \left| {{x}_{1} - {x}_{2}}\right| = \sqrt{{\left( {x}_{2} - {x}_{1}\right) }^{2} + {\left( {y}_{2} - {y}_{1}\right) }^{2}}\left| {\cos \alpha }\right| .
\]

6. 一条直线经过点 \(A\left( {2, - 3}\right)\) ,它的倾斜角等于直线 \(y = \frac{1}{\sqrt{3}}x\) 的倾斜角的 2 倍,求这条直线的方程.

7. 一根弹簧, 挂 4 公斤的物体时, 长 20 厘米, 在弹性限度内, 所挂物体的重量每增加 1 公斤, 弹簧伸长 1.5 厘米. 利用点斜式写出弹簧的长度 \(l\) (厘米) 和所挂物体重量 \(\mathbf{F}\) (公斤) 之间关系的方程.

8. 一条直线和 \(y\) 轴相交于点 \(P\left( {0,2}\right)\) ,它的倾斜角的正弦是 \(\frac{4}{5}\) . 求这条直线的方程. 这样的直线有几条?

9. 证明: 三点 \(A\left( {1,3}\right) \text{、}B\left( {5,7}\right) \text{、}C\left( {{10},{12}}\right)\) 在同一条直线上.

10. (1) 已知三角形的顶点是 \(A\left( {8,5}\right) \text{、}B\left( {4, - 2}\right) \text{、}C\left( {-6,3}\right)\) , 求经过每两边中点的三条直线的方程.

(2) \(\bigtriangleup {ABC}\) 的顶点是 \(A\left( {0,5}\right) \text{、}B\left( {1, - 2}\right) \text{、}C\left( {-6,4}\right)\) , 求 \({BC}\) 边上的中线所在直线的方程.

11. 一根铁棒在 \({40}^{ \circ }\mathrm{C}\) 时长 12.506 米,在 \({80}^{ \circ }\mathrm{C}\) 时长 12.512 米, 已知长度 \(l\) (米) 和温度 \(t\left( {{}^{ \circ }\mathrm{C}}\right)\) 的关系可以用直线方程来表示, 用两点式表示这个方程, 并且根据这个方程, 求这根铁棒在 \({100}^{ \circ }\mathrm{C}\) 时的长度.

12. 菱形的两条对角线长分别等于 8 和 6,并且分别放置在 \(x\) 轴和 \(y\) 轴上. 求菱形各边所在的直线的方程.

13. 求过点 \(P\left( {2,3}\right)\) ,并且在两轴上的截距相等的直线方程.

14. 油槽储油 20 立方米, 从一管道等速流出, 50 分钟流 完. 用截距式写出关于油槽里剩余的油量 \(Q\) (立方米) 和流出的时间 \(t\) (分) 的方程,并且画出图形 (注意: \(0 \leq t \leq {50}\) ).

15. 直线方程 \({Ax} + {By} + C = 0\) 的系数 \(A\text{、}B\text{、}C\) 满足什么关系时,这条直线 (1) 与坐标轴都相交; (2) 只与 \(x\) 轴相交; （3）只与 \(y\) 轴相交; (4) 是 \(x\) 轴; (5) 是 \(y\) 轴.

16. 设点 \(P\left( {{x}_{0},{y}_{0}}\right)\) 在直线 \({Ax} + {By} + C = 0\) 上. 求证: 这条直线的方程可以写成 \(A\left( {x - {x}_{0}}\right) + B\left( {y - {y}_{0}}\right) = 0\) .
...
*18. 某同学拿 5 元钱买纪念邮票, 票面 4 分钱的每套 5 张, 8 分钱的每套 4 张. 如果每种至少买一套, 共有几种买法, 能否恰好将钱用完, 怎样买剩钱最少?

*19. 有一滑轮组,举起的物重 \(W\) 与需用的力 \(F\) 之间的关系,由实验所得的数据如下表:
...
*20. 一个弹簧的长度 \(l\) 和它悬挂的重量 \(W\) 间关系的实验数据如下:
...
2. 求过点 \(A\left( {2,3}\right)\) ,且分别适合下列条件的直线方程:

（1）平行于直线 \({2x} + y - 5 = 0\) ;

(2) 垂直于直线 \(x - y - 2 = 0\) .

3. 已知两条直线 \({l}_{1}\text{、}{l}_{2}\) ,其中一条没有斜率. 这两条直 线什么时候 (1) 平行; (2) 垂直. 逆命题成立吗?

\section*{1. 10 两条直线所成的角}
...
1. 求下列直线 \({l}_{1}\) 到 \({l}_{2}\) 的角与 \({l}_{2}\) 到 \({l}_{1}\) 的角:
...
3. \(A\) 和 \(C\) 取什么值时,直线 \({Ax} - {2y} - 1 = 0\) 和直线 \({6x} - {4y}\) \(+ C = 0\left( 1\right)\) 平行; (2) 重合; (3) 相交.

\section*{1.12 点到直线的距离}
...
2. 设有两点 \(A\left( {7, - 4}\right) \text{、}B\left( {-5,6}\right)\) ,求线段 \({AB}\) 的垂直平分线的方程.

3. 三角形三个顶点是 \(A\left( {4,0}\right) \text{、}B\left( {6,7}\right) \text{、}C\left( {0,3}\right)\) . 求这个三角形的三条高所在直线的方程.

4. 已知直线分别满足下列条件, 求直线的方程:
...
5. 直线 \({ax} + {2y} + 8 = 0,{4x} + 3\dot{y} = {10}\) 和 \({2x} - y = {10}\) 相交于一点. 求 \(a\) 的值.

6. 不解方程组, 判定下列两个方程的直线的位置关系.
...
8. 讨论两条直线 \({l}_{1} : {A}_{1}x + {B}_{1}y + {C}_{1} = 0,{l}_{2} : {A}_{2}x + {B}_{2}y + {C}_{2}\) \(= 0\) 的位置关系: (1) 当 \({B}_{1} = 0,{B}_{2} \neq 0\) 时; (2) 当 \({B}_{1} = {B}_{2}\) \(= 0\) 时.

9. 三角形的三个顶点是 \(A\left( {6,3}\right) \text{、}B\left( {9,3}\right) \text{、}C\left( {3,6}\right)\) . 求它的三个内角的度数.

10. 已知直线 \(l\) 经过点 \(P\left( {2,1}\right)\) ,且和直线 \({5x} + {2y} + 3 = 0\) 的夹角等于 \({45}^{ \circ }\) . 求直线 \(l\) 的方程.

11. 光线从点 \(M\left( {-2,3}\right)\) 射到 \(x\) 轴上一点 \(P\left( {1,0}\right)\) 后被 \(x\) 轴反射. 求反射光线所在直线的方程.

12. 求点 \(P\left( {-5,7}\right)\) 到直线 \({12x} + {5y} - 3 = 0\) 的距离.

13. 点 \(A\left( {a,6}\right)\) 到直线 \({3x} - {4y} = 2\) 的距离 (1) 等于 4 ; (2) 大于 4. 分别求 \(a\) 的值.

14. 求证: 两条平行线 \({Ax} + {By} + {C}_{1} = 0\) 与 \({Ax} + {By} + {C}_{2} = 0\) 的距离是

\[
  d = \frac{\left| {C}_{1} - {C}_{2}\right| }{\sqrt{{A}^{2} + {B}^{2}}}
\]

15. 求两条平行线 \({3x} - {2y} - 1 = 0\) 和 \({6x} - {4y} + 2 = 0\) 的距离.
...
1. 到两坐标轴距离相等的点组成的直线的方程是 \(x - y = 0\) 吗? 为什么?

2. 已知等腰三角形三个顶点的坐标是 \(A\left( {0,3}\right) ,B\left( {-2,0}\right)\) , \(C\left( {2,0}\right)\) . 中线 \({AO}\) 的方程是 \(x = 0\) 吗? 为什么?
...
1. 求到坐标原点的距离等于 2 的点的轨迹方程.

2. 已知点 \(M\) 与 \(x\) 轴的距离和它与点 \(F\left( {0,4}\right)\) 的距离相等,求点 \(M\) 的轨迹方程。
...
1. “ \(b = 0\) ” 是 “直线 \(y = {kx} + b\) 过原点” 的什么条件,为什么?

2. “四边相等”是 “一个四边形是正方形” 的什么条件, 为什么?

3. “ \(x - 1 = 0\) ” 是 “ \({x}^{2} - 1 = 0\) ” 的什么条件,为什么?

4. “两条直线不相交”是“这两条直线异面”的什么条件, 为什么?
...
求直线 \({2x} - {5y} + 5 = 0\) 与曲线 \(y = - \frac{10}{x}\) 的交点.

\section*{习 题 四}

1. 点 \(A\left( {1, - 2}\right) \text{、}B\left( {2, - 3}\right) \text{、}C\left( {3,{10}}\right)\) 是否在方程
...
2. (1) 在什么情况下,方程 \(y = a{x}^{2} + {bx} + c\) 的曲线经过原点?

(2) 在什么情况下,方程 \({\left( x - a\right) }^{2} + {\left( y - b\right) }^{2} = {r}^{2}\) 的曲线经过原点?

3. 已知点 \(M\) 到 \(x\) 轴、 \(y\) 轴的距离的乘积等于 1 . 求点 \(M\) 的轨迹方程.

4. 点 \(M\) 到点 \(A\left( {4,0}\right)\) 和点 \(B\left( {-4,0}\right)\) 的距离的和为 12,求点 \(M\) 的轨迹方程.

5. 一个点到点 \(\left( {4,0}\right)\) 的距离等于它到 \(y\) 轴的距离,求这个点的轨迹方程.

6. 两个定点的距离为 6,点 \(M\) 到这两个定点的距离的平方和为 26,求点 \(M\) 的轨迹方程.

7. 求与点 \(O\left( {0,0}\right)\) 和 \(A\left( {c,0}\right)\) 的距离的平方差为常数 \(c\) 的点的轨迹方程.
...
8. 两根杆分别绕着定点 \(A\) 和 \(B\left( {{AB} = {2a}}\right)\) 在平面内转动, 并且转动时两杆保持相互垂直, 求杆的交点 \(P\) 的轨迹方程.

9. 在下列括弧中填写: “充分条件”或“必要条件” 或 “充要条件”:
...
10. 求直线 \({4x} - {3y} = {20}\) 和圆 \({x}^{2} + {y}^{2} = {25}\) 的交点.

11. 求经过两条曲线 \({x}^{2} + {y}^{2} + {3x} - y = 0\) 和 \(3{x}^{2} + 3{y}^{2} + {2x} + y\) \(= 0\) 交点的直线的方程.
...
2. 一个圆过点 \(P\left( {{12},0}\right)\) ,且与 \(y\) 轴切于原点. 求这个圆的方程,并判断点 \(A\left( {6, - 6}\right) \text{、}B\left( {5, - 5}\right) \text{、}C\left( {{2.5},5}\right)\) 是在圆内, 在圆外, 还是在圆上.

3. 已知一个圆的圆心在原点,并与直线 \({4x} + {3y} - {70} = 0\) 相切. 求圆的方程.

4. 写出过圆 \({x}^{2} + {y}^{2} = {10}\) 上一点 \(M\left( {2,\sqrt{6}}\right)\) 的切线的方程.

5. 已知圆的方程是 \({x}^{2} + {y}^{2} = 1\) . 求:
...
3. 已知: 一个圆的直径端点是 \(A\left( {{x}_{1},{y}_{1}}\right) \text{、}B\left( {{x}_{2},{y}_{2}}\right)\) . 证明: 圆的方程是 \(\left( {x - {x}_{1}}\right) \left( {x - {x}_{2}}\right) + \left( {y - {y}_{1}}\right) \left( {y - {y}_{2}}\right) = 0\) .

4. 一个等腰三角形底边上的高等于 5 , 底边两端点的坐标是 \(\left( {-4,0}\right)\) 和 \(\left( {4,0}\right)\) . 求它的外接圆的方程.

5. 赵州桥的 跨度是 37.4 米, 圆拱高约为 7.2 米. 求 这座圆拱桥的拱圆的方程.
...
6. 某工件尺寸如图所示, 它的外缘由四段圆弧连接而成. 求各段圆弧所在的圆的方程.

7. 求通过点 \(A\left( {1,2}\right)\) ,且与两坐标轴同时相切的圆的方程.

8. 过点 \(A\left( {0,\sqrt{10}}\right)\) 向圆 \({x}^{2} + {y}^{2} = 5\) 引两条切线. 求它们的方程.
...
10. 求下列各圆的圆心坐标和半径, 并画出它们的图形:

(1) \({x}^{2} + {y}^{2} - {2x} - 5 = 0\) ; (2) \({x}^{2} + {y}^{2} + {2x} - {4y} - 4 = 0\) ;

(3) \({x}^{2} + {y}^{2} + {2ax} = 0\) ; (4) \({x}^{2} + {y}^{2} - {2by} - 2{b}^{2} = 0\) .

11. 求证: 两圆 \({x}^{2} + {y}^{2} - {4x} - {6y} + 9 = 0\) 和 \({x}^{2} + {y}^{2} + {12x} + {6y}\) \(- {19} = 0\) 相外切.

12. 求直线 \({4x} - {3y} = {50}\) 和圆 \({x}^{2} + {y}^{2} = {100}\) 的交点,说明它们的位置关系.

13. 求经过两圆 \({x}^{2} + {y}^{2} + {6x} - 4 = 0\) 和 \({x}^{2} + {y}^{2} + {6y} - {28} = 0\) 的交点,并且圆心在直线 \(x - y - 4 = 0\) 上的圆的方程.

14. 等腰三角形的顶点是 \(A\left( {4,2}\right)\) ,底边一个端点是 \(B\left( {3,5}\right)\) . 求另一个端点的轨迹方程, 并说明它的轨迹是什么.

15. 一条线段 \({AB}\left( {{AB} = {2a}}\right)\) 的两个端点 \(A\) 和 \(B\) 分别在 \(x\) 轴和 \(y\) 轴上滑动. 求线段 \({AB}\) 的中点 \(M\) 的轨迹方程.

\section*{三 椭 圆}

\section*{2.7 椭圆及其标准方程}
...
3. 已知 \(\bigtriangleup {ABC}\) 的一边 \({BC}\) 长为 6,周长为 16 . 求顶点 \(A\) 的
...
1. 说出椭圆 \(\frac{{y}^{2}}{{a}^{2}} + \frac{{x}^{2}}{{b}^{2}} = 1\) 的焦点和顶点的坐标.

2. 求下列各椭圆的长轴和短轴的长、离心率、焦点坐标、顶点坐标和准线方程, 并画出草图:
...
1. 如图,椭圆上的点中, \({A}_{1}\) 与焦点 \({F}_{1}\) 的距离最小, \(\left| {{A}_{1}{F}_{1}}\right|\) \(= 2,{A}_{2}\) 与 \({F}_{1}\) 的距离最大, \(\left| {{A}_{2}{F}_{1}}\right| = {14}\) ,求椭圆的标准方程。

2. 已知椭圆的面积公式是 \(S = {\pi ab}\) ,其中 \(a\text{、}b\) 分别是椭圆长半轴和短半轴的长. 利用这个公式, 求下列椭圆的面积:

(1) \(9{x}^{2} + {y}^{2} = 8\) ; (2) \(9{x}^{2} + {25}{y}^{2} = {100}\) .

3. 求椭圆 \(\frac{{x}^{2}}{16} + \frac{{y}^{2}}{25} = 1\) 上一点 \({M}_{1}\left( {{2.4},4}\right)\) 与焦点的距离.
...
6. 我国发射的科学实验人造地球卫星的运行轨道是以地球的中心为一个焦点的椭圆, 近地点距地面 266 公里, 远地点距地面 1826 公里. 求这颗卫星的轨道方程.

7. 彗星“紫金山一号”是南京天文台发现的. 它的运行轨道是以太阳为一个焦点的椭圆. 测得轨道的近日点距太阳中心 1.486 天文单位, 远日点距太阳中心 5.563 天文单位 (1 天文单位是太阳到地球的平均距离, 约 1.5 亿公里), 求椭圆的方程.

8. 已知地球运行的轨道是长半轴长 \(a = {1.50} \times {10}^{8}\mathrm{\;{km}}\) ,离心率 \(e = {0.0192}\) 的椭圆. 太阳在这个椭圆的一个焦点上. 求地球到太阳最大和最小的距离.
...
10. 点 \(P\) 与一定点 \(F\left( {2,0}\right)\) 的距离和它到一定直线 \(x = 8\) 的距离的比是 1:2. 求点 \(P\) 的轨迹方程,并说明轨迹是什么图形.

11. \(\bigtriangleup {ABC}\) 的一边的两顶点是 \(B\left( {0,6}\right)\) 和 \(C\left( {0, - 6}\right)\) ,另两边的斜率的乘积是 \(- \frac{4}{9}\) ,求顶点 \(A\) 的轨迹.

12. 点 \(M\) 与椭圆 \(\frac{{x}^{2}}{{13}^{2}} + \frac{{y}^{2}}{{12}^{2}} = 1\) 的左焦点和右焦点的距离的比为 2:3. 求点 \(M\) 的轨迹方程,并画出图形.

13. 已知直线和椭圆的方程如下, 求它们的交点坐标:

(1) \({3x} + {10y} - {25} = 0,\frac{{x}^{2}}{25} + \frac{{y}^{2}}{4} = 1\) ;

(2) \({3x} - y + 2 = 0,\frac{{x}^{2}}{16} + \frac{{y}^{2}}{4} = 1\) .

14. 求证两椭圆 \({b}^{2}{x}^{2} + {a}^{2}{y}^{2} - {a}^{2}{b}^{2} = 0,{a}^{2}{x}^{2} + {b}^{2}{y}^{2} - {a}^{2}{b}^{2} = 0\) 的交点在以原点为中心的圆周上, 并求这个圆的方程.

\section*{四双曲线}

\section*{2.9 双曲线及其标准方程}
...
2. 证明: 椭圆 \(\frac{{x}^{2}}{25} + \frac{{y}^{2}}{9} = 1\) 与双曲线 \({x}^{2} - {15}{y}^{2} = {15}\) 的焦点相同。

\section*{2.10 双曲线的几何性质}
...
3. 等轴双曲线的一个焦点是 \({F}_{1}\left( {-6,0}\right)\) ,求它的标准方程和渐近线方程.
...
2. 参考椭圆的画法, 根据双曲线的定义, 研究一下怎样用直尺和圆规作点画出双曲线.
...
5. 求与椭圆 \(\frac{{x}^{2}}{49} + \frac{{y}^{2}}{24} = 1\) 有公共焦点,且离心率 \(e = \frac{5}{4}\) 的双曲线方程.

6. 求以椭圆 \(\frac{{x}^{2}}{8} + \frac{{y}^{2}}{5} = 1\) 的焦点为顶点,而以椭圆的顶点为焦点的双曲线方程.

7. 求与双曲线 \(\frac{{x}^{2}}{9} - \frac{{y}^{2}}{16} = 1\) 有共同的渐近线,且经过点 \(A( - 3\) , \(2\sqrt{3}\) ) 的双曲线方程.

8. 求经过点 \(A\left( {3, - 1}\right)\) ,并且对称轴都在坐标轴上的等轴双曲线的方程.

9. 证明: 等轴双曲线的离心率是 \(\sqrt{2}\) .

10. 在相距 1400 米的 \(A\text{、}B\) 两哨所,听到炮弹爆炸声的时间相差 3 秒, 已知声速是 340 米/秒. 炮弹爆炸点在怎样的曲线上?

11. 通过双曲线 \(\frac{{x}^{2}}{144} - \frac{{y}^{2}}{25} = 1\) 的一个焦点,作 \(x\) 轴的垂线,求垂线与双曲线的交点与两焦点的距离.

12. 证明从双曲线的一个焦点到一条渐近线的距离等于虚半轴长.
...
14. 证明椭圆 \(\frac{{x}^{2}}{20} + \frac{{y}^{2}}{5} = 1\) 与双曲线 \(\frac{{x}^{2}}{12} - \frac{{y}^{2}}{3} = 1\) 的交点是一矩形的顶点.

15. 求与定点 \(A\left( {5,0}\right)\) 及定直线 \(l : x = \frac{16}{5}\) 的距离比是 5:4 的点的轨迹方程.

16. \(\bigtriangleup {ABC}\) 一边的两个端点是 \(B\left( {0,6}\right)\) 和 \(C\left( {0, - 6}\right)\) ,另两边斜率的积是 \(\frac{4}{9}\) ,求顶点 \(A\) 的轨迹.

17. 判定当 \(\left( 1\right) k < 4;\left( 2\right) 4 < k < 9\) 时,方程 \(\frac{{x}^{2}}{9 - k} + \frac{{y}^{2}}{4 - k} = 1\) 分别表示什么曲线.

18. 当 \(\alpha\) 从 \({0}^{ \circ }\) 到 \({180}^{ \circ }\) 变化时,曲线 \({x}^{2} + {y}^{2}\cos \alpha = 1\) 怎样变化?

\section*{五 抛 物 线}

\section*{2.11 抛物线及其标准方程}
...
1. 根据抛物线的定义, 仿照椭圆和双曲线的画法, 利用直尺和圆规作点画出焦点到准线的距离等于 \(p\) 的抛物线.

2. 根据下列所给的条件, 写出抛物线的标准方程:
...
2. 一条隧道的顶部是抛物拱形, 拱高是 1.1 米, 跨度是 2.2 米. 求拱形的抛物线方程.

3. 证明: 与抛物线的轴平行的直线和抛物线只有一个交点.

\section*{习 题 八}

1. 抛物线 \({y}^{2} = {2px}\left( {p > 0}\right)\) 上一点 \(M\) 到焦点的距离是 \(a\) \(\left( {a > \frac{p}{2}}\right)\) ,点 \(M\) 到准线的距离是多少? 点 \(M\) 的横坐标是多少?

2. 取经过焦点 \(F\) 且垂直于准线 \(l\) 的直线为 \(y\) 轴,推导抛物线的标准方程.
...
4. 根据下列条件, 求抛物线的方程, 并描点画出图形:

（1）顶点在原点,对称轴是 \(x\) 轴,并且顶点与焦点的距离等于6;

（2）顶点在原点,对称轴是 \(y\) 轴,并经过点 \(P\left( {-6, - 3}\right)\) .

5. 经过抛物线 \({y}^{2} = {2px}\) 的焦点 \(F\) ,作一条直线垂直于它的对称轴,和抛物线相交于 \({P}_{1}\text{、}{P}_{2}\) 两点,线段 \({P}_{1}{P}_{2}\) 叫做抛物线的通径. 求通径 \({P}_{1}{P}_{2}\) 的长.

6. 在抛物线 \({y}^{2} = {12x}\) 上,求和焦点的距离等于 9 的点的坐标.

7. 有一正三角形的两个顶点在抛物线 \({y}^{2} = {2px}\) 上,另一顶点在原点. 求这个三角形的边长.

8. 过抛物线 \({y}^{2} = {2px}\) 的焦点的一条直线和这抛物线相交,两个交点的纵坐标为 \({y}_{1}\text{、}{y}_{2}\) . 求证: \({y}_{1}{y}_{2} = - {p}^{2}\) .

9. 吊车梁的鱼腹部分 \({AOB}\) 是一段抛物线,宽为 7 米,高为 0.7 米, 求这条抛物线的方程。
...
10. 图中是抛物线形拱桥,当水面在 \(l\) 时,拱顶离水面 2 米, 水面宽 4 米. 水下降 1 米后, 水面宽多少?

11. 抛物线的顶点是双曲线 \({16}{x}^{2} - 9{y}^{2} = {144}\) 的中心,而焦点是双曲线的左顶点. 求抛物线的方程.

12. 已知两抛物线的顶点在原点,而焦点分别在点 \({F}_{1}\left( {2,0}\right)\) 和点 \({F}_{2}\left( {0,2}\right)\) . 求它们的交点.

13. 过抛物线焦点的一条直线与它交于两点 \(P\text{、}{Q}_{8}\) . 通过点 \(P\)

和抛物线顶点的直线交准线于点 \(M\) ,求证: 直线 \({MQ}\) 平行于抛物线的对称轴.

14. 点 \(M\) 与点 \(F\left( {4,0}\right)\) 的距离比它到直线 \(x + 5 = 0\) 的距离小 1,求点 \(M\) 的轨迹方程,并且画出图形.

\section*{*2. 13 圆锥曲线的切线和法线}
...
2. 过点 \(P\left( {-3,5}\right)\) 作抛物线 \({y}^{2} = {8x}\) 的切线. 求切线的方程。
...
2. 求经过点 \(P\left( {-2, - 1}\right)\) 的椭圆 \(5{x}^{2} + {y}^{2} = 5\) 的切线方程.

3. 已知双曲线 \(\frac{{x}^{2}}{10} - \frac{{y}^{2}}{6} = 1\) 的一条切线的斜率等于 2 . 求这条切线的方程.

4. 双曲线 \(\frac{{x}^{2}}{4} - \frac{{y}^{2}}{5} = 1\) 的切线平行于直线 \({3x} - {2y} = 0\) ,求切线的方程.

5. \(k\) 为何值时,直线 \({kx} - y - 1 = 0\) 与抛物线 \({x}^{2} = {4y}\left( 1\right)\) 相交; (2) 相切; (3) 相离?

6. 已知经过抛物线 \({y}^{2} = {2px}\) 上两点 \({P}_{1}\left( {{x}_{1},{y}_{1}}\right)\) 和 \({P}_{2}\left( {{x}_{2},{y}_{2}}\right)\) 的两条切线相交于点 \(M\left( {{x}_{0},{y}_{0}}\right)\) . 求证: \({x}_{0} = \frac{{y}_{1}{y}_{2}}{2p}\) , \({y}_{0} = \frac{{y}_{1} + {y}_{2}}{2}\)

7. 求抛物线 \({y}^{2} = {9x}\) 和圆 \({x}^{2} + {y}^{2} = {36}\) 在交点处的切线方程, 并求出这两条切线的夹角.

8. 椭圆 \({x}^{2} + 4{y}^{2} = 4\) 的一条切线经过点 \(P\left( {-\sqrt{2},\frac{\sqrt{2}}{2}}\right)\) ,求焦点到这条切线的距离.

9. 防空探照灯的反射镜与过轴的截面的交线是一条抛物线. 已知灯 \(\mu\) 直径是 \({2000}\mathrm{\;{mm}}\) ,深度是 \({800}\mathrm{\;{mm}}\) . 光源应放在什么位置?

10. 有一条光线从抛物线 \({y}^{2} = {4x}\) 的焦点射出,射到抛物线上的点 \(P\left( {9,6}\right)\) ,求入射光线和反射光线所在的直线的方程.
...
1. 平移坐标轴,把原点移到 \({O}^{\prime }\left( {4,5}\right)\) . 求下列各点的新坐标, 并画出新坐标轴和各点:

\[
  A\left( {3, - 6}\right) \text{、}B\left( {7,0}\right) \text{、}C\left( {-4,5}\right) \text{、}D\left( {0, - 8}\right) .
\]

2. 平移坐标轴,把原点移到 \({O}^{\prime }\left( {2, - 3}\right)\) . 求 \({x}^{2} + {y}^{2} - {4x} + {6y}\) \(- 3 = 0\) 在新坐标系中的方程,并画出新坐标轴和图形.

\section*{3.2 利用坐标轴的平移化简二元二次方程}
...
1. 经过怎样的平移变换,可以把方程 \({x}^{2} + {y}^{2} - {6x} + {12y} - 4\) \(= 0\) 化为没有一次项的新方程?

2. 化简方程 \(y = \frac{1}{2}{x}^{2} + x + \frac{5}{2}\) ,并画出它的图形.

\section*{习 题 十}

1. (1) 平移坐标轴, 把原点分别移到何处, 点的坐标变化如下:
...
2. 平移坐标轴,把原点移到 \({O}^{\prime }\) ,求下列各曲线的新方程,并画出新坐标轴和图形.
...
1. 设旋转角 \(\theta = - \frac{\pi }{4}\) ,求新坐标系中的两点 \(A\left( {-3,2}\right) ,B(1\) , 0) 在原坐标系的坐标.

2. 设旋转角 \(\theta = \frac{\pi }{6}\) ,求原坐标系中的两点 \(C\left( {2, - 1}\right) ,D\left( {0,1}\right)\) 在新坐标系的坐标.
...
1. 把点 \(A\left( {4,3}\right)\) 的坐标变成 \(\left( {3,4}\right)\) ,坐标轴应旋转多大角度?

2. 坐标轴旋转多大角度,才能使点 \(M\left( {1,2 + \sqrt{3}}\right)\) 的横、纵坐标相等? 求出点 \(M\) 的新坐标.

3. 将坐标轴旋转 \(\frac{\pi }{3}\) 后,点 \(M\) 的坐标变为 \(\left( {1,2}\right)\) . 求点 \(M\) 在原坐标系的坐标.

4. 将坐标轴旋转 \(- \frac{\pi }{2}\) ,求椭圆 \({x}^{2} + \frac{{y}^{2}}{2} = 1\) 在新坐标系中的方程.

5. 将坐标轴旋转 \(\frac{\pi }{2}\) ,求抛物线 \({y}^{2} = {2px}\) 在新坐标系中的方程.

6. 证明: 不论坐标轴旋转多大角度,方程 \({x}^{2} + {y}^{2} = {r}^{2}\) 的形式不变.

7. 利用坐标轴旋转, 化简下列方程, 并画出图形:
...
1. 化简方程 \(3{x}^{2} - {10xy} + 3{y}^{2} + {26x} - {22y} + {35} = 0\) .

2. 化简方程 \({4xy} - 3{x}^{2} + 4 = 0\) ,画出它的图形.

\section*{3. 6 一般二元二次方程的讨论}
...
5. 方程 \(2{x}^{2} + {\lambda xy} + 4{y}^{2} - {7x} + {\lambda }^{2}y + 3 = 0\) 中, \(\lambda\) 取什么值时,

方程是: (1) 椭圆型; (2) 双曲线型; (3) 抛物线型?

6. 判断下列方程有没有中心, 如果有, 求出中心坐标:
...
1. 求半径为 \(r\) 、圆心在原点 \(O\) 的圆的参数方程.

(提示: 以 \(\phi\) 为参数,如图.)

2. 已知一条直线上两点 \({M}_{1}\left( {{x}_{1},{y}_{1}}\right) \text{、}{M}_{2}\left( {{x}_{2},{y}_{2}}\right)\) ,以分点 \(M\) \(\left( {x,y}\right)\) 分 \(\overline{{M}_{1}{M}_{2}}\) 所成的比 \(\lambda\) 为参数,写出参数方程.

\section*{4.2 参数方程和普通方程的互化}
...
1. 把下列参数方程 \(\left( {\phi \text{、}t\text{是参数}}\right)\) 化成普通方程,并说明它们各表示什么曲线:
...
1. 将圆的渐开线中的基圆换为正方形就得到正方形的渐开线. 用刻度尺、圆规画出边长为 \(1\mathrm{\;{cm}}\) 的正方形的渐开 线.

2. 如图, 有一标准的渐开线齿轮, 齿轮的齿廓线的基圆直径是 \({225}\mathrm{\;{mm}}\) ,求齿廓线 \({AB}\) 所在渐开线的参数方程.
...
1. 求摆线 \(\left\{ \begin{array}{l} x = \phi - \sin \phi , \\ y = 1 - \cos \phi \end{array}\right.\) 上 \({2\pi } \leq \phi \leq {4\pi }\) 这一段与直线

\(y = \frac{3}{2}\) 的交点的坐标.

2. 有一轮子, 沿着直线轨道滚动, 轮子的半径是 \(a\) ,在轮辐上有一点 \(M\) ,与轮子中心的距离是 \(b\left( {b < a}\right)\) . 点 \(M\) 的轨迹叫短摆线, 求它的参数方程.
...
2. 把下列参数方程化成普通方程 (其中 \(t\text{、}\phi\) 是参数),并说明各表示什么曲线:
...
3. 根据所给的条件,化下列方程成参数方程 \((t\) 和 \(\phi\) 是参数):

(1) \({y}^{2} = 4{x}^{2} - 5{x}^{3},\;y = {tx}\) ;

(2) \(4{x}^{2} + {y}^{2} - {16x} + {12} = 0,\;y = 2\sin \phi\) .

4. 设 \(x = 2\cos \phi ,\phi\) 是参数,求椭圆 \(4{x}^{2} + {y}^{2} = {16}\) 的参数方程.

5. 动点 \(M\) 作等速直线运动,它在 \(x\) 轴和 \(y\) 轴方向的分速度分别为 9 和 12,运动开始时,点 \(M\) 位于 \(A\left( {1,1}\right)\) ,求点 \(M\) 的轨迹的参数方程.

6. (1) 写出经过点 \(M\left( {1,5}\right)\) ,倾斜角是 \(\frac{\pi }{3}\) 的直线的参数方程.

（2）利用这个参数方程,求这条直线和直线 \(x - y - 2\sqrt{3}\) \(= 0\) 的交点到点 \(M\) 的距离;

（3）求这条直线和圆 \({x}^{2} + {y}^{2} = {16}\) 的两个交点到点 \(M\) 的距离的和与积.

7. 椭圆规是用来画椭圆的一种器械, 它的构造如图所示.
...
8. 一颗人造地球卫星的运行轨道是一个椭圆, 长轴长为 15565 公里, 短轴长为 15443 公里, 取椭圆中心为坐标原点, 求卫星轨道的参数方程.

9. (1) 物体从高处以初速 \({v}_{0}\) 米/秒沿水平方向抛出. 以抛出点为原点,过抛出点的水平直线为 \(x\) 轴,写出物体所经路线的参数方程.

（2）作水平飞行的飞机的速度 \(v = {150}\) 米 \(/\) 秒,若在飞行高度 \(H = {600}\) 米时投弹,求 (i) 炸弹离开飞机后的轨迹方程; (ii) 飞机在离目标多远 (水平距离) 的地方投弹才能命中目标?

*10. 一个圆沿一条直线滚动时,点 \(M\) 是圆的一条半径延长线上的一点. 求点 \(M\) 的轨迹的参数方程.
...
1. 写出图中 \(A\text{、}B\text{、}C\text{、}D\text{、}E\text{、}F\text{、}G\) 各点的极坐标 \((\rho > 0,0 \leq \theta\) \(< {2\pi })\) .
...
3. 在极坐标系中,画出点 \(A\left( {5,\frac{\pi }{3}}\right)\) 以及点 \(B\left( {5, - \frac{\pi }{3}}\right)\) 、

\(C\left( {-5, - \frac{\pi }{3}}\right) ,D\left( {-5,\frac{\pi }{3}}\right)\) ,并说明点 \(A\) 和 \(B\text{、}C\text{、}D\) 分别有. 怎样的相互位置关系.

\section*{4.5 曲线的极坐标方程}
...
1. 以圆锥曲线的焦点为极点, 焦点到准线的垂线的反向延长线为极轴, 写出下列圆锥曲线的极坐标方程:
...
1. 已知各点的极坐标为 \(\left( {4,\frac{\pi }{4}}\right) ,\left( {2,\frac{4\pi }{3}}\right) ,\left( {-7,\pi }\right) ,\left( {5,\frac{\pi }{2}}\right)\) 、

\(\left( {-2, - \frac{\pi }{6}}\right)\) ,求它们的直角坐标.

2. 已知各点的直角坐标为 \(\left( {-1, - 1}\right) \text{、}\left( {4, - 4\sqrt{3}}\right) \text{、}( - 3\) , \(4)\text{、}\left( {0, - 4}\right)\) ,求它们的极坐标.
...
1. 求证: 如果 \({M}_{1}\left( {{\rho }_{1},{\theta }_{1}}\right) \text{、}{M}_{2}\left( {{\rho }_{2},{\theta }_{2}}\right)\) 是等速螺线上的两点, 那么 \({\rho }_{2} - {\rho }_{1}\) 与 \({\theta }_{2} - {\theta }_{1}\) 成正比例.
...
2. 某自动机床上有一个凸轮, 它的轮廓线 \({ACB}\) 是一段等速螺线 (如图), \(A\) 点到旋转中心 \(O\) 的距离 \({\rho }_{0}\) \(= {60}\) 毫米,轴心角 \(\angle {AOB} = {30}^{ \circ }\) , 工作时曲线 \({ACB}\) 能把从动杆推出 5 毫米. 求这段等速螺线的极坐标方程.

3. 当 \(0 \geq \theta \geq - \frac{5\pi }{8}\) 时,求本节例 2 中的曲线 \({ADC}\) 的方程.

\section*{习 题 十 四}

1. 已知 \(A\left( {\rho ,\theta }\right) \text{、}B\left( {\rho , - \theta }\right) \text{、}C\left( {-\rho , - \theta }\right) \text{、}D\left( {-\rho ,\theta }\right)\) ,点 \(A\) 和 \(B\text{、}C\text{、}D\) 分别有怎样的相互位置关系?
...
5. 从极点作圆 \(\rho = {2a}\cos \theta\) 的弦,求各个弦的中点的轨迹方程.

6. 从极点 \(O\) 作直线和直线 \(\rho \cos \theta = 4\) 相交于点 \(M\) ,在 \({OM}\) 上取一点 \(P\) ,使 \({OM} \cdot {OP} = {12}\) ,求 \(P\) 点的轨迹的方程,并且说明轨迹是什么曲线.

7. 一颗彗星的轨道是抛物线, 太阳位于这条抛物线的焦点上. 已知这颗彗星距太阳 \({1.6} \times {10}^{8}\) 公里时,极径和轨道的轴成 \(\frac{\pi }{3}\) 角. 求这颗彗星轨道的极坐标方程,并且求它的近日点离太阳的距离.
...
10. 已知一个圆的方程是 \(\rho = 5\sqrt{3}\cos \theta - 5\sin \theta\) ,求圆心和半径.

11. 长为 \({2a}\) 的线段,其端点在两个直角坐标轴上滑动,从原点作这条线段的垂线,垂足为 \(M\) ,求点 \(M\) 的轨迹的极坐标方程 ( \({Ox}\) 为极轴),再化为直角坐标方程.

12. 一凸轮如图所示, 当它按箭头方向等速转动时, 要求:
